...
import { categoryBarsSvg } from "../charts.js";
...
function downloadHtml(state) {
  ...
  <div style="margin-bottom:20px;">
    <button type="button" class="btn-primary" id="informe-download" ${state.downloading ? "disabled" : ""}>${state.downloading ? t("informe.downloading") : t("informe.download")}</button>
...
function summaryHtml(report) {
  ...
  // La segunda frase («En agosto, el 43 %») necesitaría el ahorro del periodo ANTERIOR, que
  // repo.reportInputs no trae (evita duplicar dos consultas más solo para esta línea, spec §5.1):
  // s.prevSavingsRatePct queda siempre en null y la frase se omite, nunca "el X% / null%".
  const sentence = s.savingsRatePct != null ? t("informe.summary.savingsRate", { pct: s.savingsRatePct }) : "";
  return `
...
function accountsHtml(report) {
  ...
  return `
  <div style="margin-bottom:24px;">
    <div class="section-title" style="margin-bottom:10px;">${t("informe.accounts.title")}</div>
    <div style="display:flex;flex-direction:column;gap:10px;">
      ${a.rows.map((r) => `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
        <span style="font-size:14px;font-weight:500;">${escHtml(r.name)}</span>
        <span class="num" style="font-size:13px;color:${r.deltaCents < 0 ? "var(--danger)" : "var(--pos)"};">${escHtml(fmtMoney(r.startCents))} → ${escHtml(fmtMoney(r.endCents))}</span>
      </div>`).join("")}
      <hr class="divider">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
        <span style="font-size:14px;font-weight:700;">${t("informe.accounts.total")}</span>
        <span class="num" style="font-size:14px;font-weight:700;color:${a.totalDeltaCents < 0 ? "var(--danger)" : "var(--pos)"};">${escHtml(fmtMoney(a.totalStartCents))} → ${escHtml(fmtMoney(a.totalEndCents))}</span>
      </div>
    </div>
  </div>`;
}
...
function categoriesHtml(report, prevPeriodName) {
  ...
  const rows = c.rows.map((r) => {
    const bar = categoryBarsSvg([{ key: r.rootId, value: r.spentCents, max: Math.max(0, ...c.rows.map((x) => x.spentCents)), color: r.color }],
    ...
    <div style="display:flex;flex-direction:column;gap:6px;">
      ...
      ${bar}
      ${c.hasPrev && r.prevCents != null ? `<div style="font-size:11px;color:var(--ink-3);">${escHtml(fmtMoney(r.prevCents))} ${trend}</div>` : ""}
    </div>`;
  }).join("");
...
function sharedHtml(report) {
  ...
  <div style="margin-bottom:24px;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
      ...
      <span class="num" style="font-size:14px;font-weight:600;">${escHtml(fmtMoney(s.myPartCents))} / ${escHtml(fmtMoney(s.periodTotalCents))}</span>
    </div>
...
function subscriptionsHtml(report) {
  ...
  return `
  <div style="margin-bottom:24px;">
    <div class="section-title" style="margin-bottom:10px;">${t("informe.subscriptions.title")}</div>
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
    ...
    <button type="button" id="informe-suscripciones-link" class="link-btn" style="margin-top:10px;">${t("informe.subscriptions.link")}</button>
  </div>`;
}
...
function movementGroupHtml(g, expanded) {
  ...
  <div style="display:flex;flex-direction:column;gap:6px;">
    <button type="button" data-group="${escAttr(g.rootId)}" aria-expanded="${expanded ? "true" : "false"}"
      ...
      <span style="flex:1;min-width:0;font-size:14px;font-weight:600;">${escHtml(g.name)}</span>
